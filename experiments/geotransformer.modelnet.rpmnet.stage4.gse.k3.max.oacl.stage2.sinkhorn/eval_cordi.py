...
class EncoderTrainer(IterBasedEncoderTrainer):
    def __init__(self, cfg):
        ...
        cordi = None
        if cfg.ddpm.use_ddpm_inference:
            # load pretrained ddpm model
            cordi = create_cordi(cfg)
            self.snapshot_ddpm_dir = cfg.snapshot_ddpm_dir
            snapshot = osp.join(self.snapshot_ddpm_dir, 'snapshot.pth.tar')
            state_dict = torch.load(snapshot, map_location=torch.device('cpu'))
            model_dict = state_dict['model']
            cordi.load_state_dict(model_dict, strict=False)
            self.logger.info('Pretrained ddpm model has been loaded.')

        # model, optimizer, scheduler
        model = create_model(cfg, cordi).cuda()
