...
class GeoTransformer(nn.Module):
    ...
    def forward(self, data_dict):
        ...
        with torch.no_grad():
            ref_node_corr_indices, src_node_corr_indices, node_corr_scores, ref_no_match_indices, src_no_match_indices, node_match_scores = self.coarse_matching(
            ...
            # normalize node_corr_scores to 0 ~ 1
            output_dict['node_corr_score'] = (node_match_scores - torch.min(node_match_scores)) / (torch.max(node_match_scores) - torch.min(node_match_scores))

            ref_node_corr_indices_m, src_node_corr_indices_m, _, _, _, _ = self.coarse_matching_m(
                ref_feats_sel_c_norm, src_feats_sel_c_norm, ref_node_masks_sel, src_node_masks_sel
            )
            ...
            ref_node_corr_indices_s, src_node_corr_indices_s, _, _, _, _ = self.coarse_matching_s(
                ref_feats_sel_c_norm, src_feats_sel_c_norm, ref_node_masks_sel, src_node_masks_sel
            )
