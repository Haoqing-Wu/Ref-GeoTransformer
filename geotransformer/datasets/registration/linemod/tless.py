...
class TLessDataset(data.Dataset):
    ...
    def __init__(
        ...
        super(TLessDataset, self).__init__()

        # root dir
        self.base_dir = data_folder + 'tless/'
        ...
        pickle_files = {os.path.splitext(os.path.basename(file))[0]:\
                                str(file) for file in Path(self.pickle_root).glob('tless_{0}*.pkl'.format(self.mode))}
        for file in pickle_files:
            if self.overfit is not None and self.overfit != int(file.split('_')[-1]) and self.mode == 'train':
                continue
    ...
    def __getitem__(self, index):
        ...
        model_info_root = self.base_dir + 'models/models_cad/models_info.json'
    ...
    def get_dataset_from_path(self):
        ...
        model_root = self.base_dir + 'models/models_cad'
        ...
        if self.mode == 'train':
            ...
            for obj_id in tqdm(range(obj_num)):
                ...
                xmap = np.array([[j for i in range(1280)] for j in range(1024)])
                ymap = np.array([[i for i in range(1280)] for j in range(1024)])
                ...
                for frame_id in frames:
                    ...
                    rmin, rmax, cmin, cmax = get_bbox(mask_to_bbox(mask), 'tless')
            ...
            for scene_id in tqdm(range(first, last)):
                ...
                with open(self.pickle_root + 'tless_{0}_{1}.pkl'.format(self.mode, str(scene_id)), 'wb') as f:
                    pickle.dump(data, f)
